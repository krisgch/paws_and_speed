...
-- ⚠️  This replaces the old single-table (competitions) schema.
...
-- Hosts and super admins can read all profiles
CREATE POLICY "profiles: hosts read all" ON public.profiles
  FOR SELECT USING (
...
-- Only hosts / super admins can see invite codes
CREATE POLICY "host_invites: hosts read" ON public.host_invites
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.profiles p WHERE p.id = auth.uid() AND p.role IN ('host', 'super_admin'))
  );

-- RPC: redeem_host_invite — called from Signup page
...
CREATE POLICY "dogs: owner CRUD" ON public.dogs
  FOR ALL USING (auth.uid() = owner_id);

CREATE POLICY "dogs: hosts read all" ON public.dogs
  FOR SELECT USING (public.is_super_admin() OR
...
-- Host: full CRUD on own events; super_admin: full CRUD on all events
CREATE POLICY "events: host CRUD own" ON public.events
  FOR ALL USING (auth.uid() = host_id OR public.is_super_admin());

-- Public: read non-draft events
CREATE POLICY "events: public read non-draft" ON public.events
...
-- Host manages their own event's rounds; super_admin manages all
CREATE POLICY "event_rounds: host write" ON public.event_rounds
  FOR ALL USING (
    public.is_super_admin() OR
    EXISTS (SELECT 1 FROM public.events e WHERE e.id = event_id AND e.host_id = auth.uid())
  );

-- Public reads rounds for non-draft events
CREATE POLICY "event_rounds: public read" ON public.event_rounds
  FOR SELECT USING (
...
-- Competitor: insert own, update receipt on own pending_payment rows
CREATE POLICY "registrations: competitor insert" ON public.registrations
  FOR INSERT WITH CHECK (auth.uid() = competitor_id);

CREATE POLICY "registrations: competitor read own" ON public.registrations
  FOR SELECT USING (auth.uid() = competitor_id);

CREATE POLICY "registrations: competitor update receipt" ON public.registrations
  FOR UPDATE USING (auth.uid() = competitor_id AND status = 'pending_payment');

-- Host: read + update (approve/reject) registrations for their events; super_admin: all
CREATE POLICY "registrations: host manage" ON public.registrations
  FOR ALL USING (
    public.is_super_admin() OR
    EXISTS (SELECT 1 FROM public.events e WHERE e.id = event_id AND e.host_id = auth.uid())
  );
...
-- Host: full write access for their events; super_admin: all events
CREATE POLICY "event_competitors: host write" ON public.event_competitors
  FOR ALL USING (
    public.is_super_admin() OR
    EXISTS (SELECT 1 FROM public.events e WHERE e.id = event_id AND e.host_id = auth.uid())
  );

-- Public: read competitors for live/completed events
CREATE POLICY "event_competitors: public read" ON public.event_competitors
  FOR SELECT USING (
...
CREATE POLICY "event_live_state: host write" ON public.event_live_state
  FOR ALL USING (
    public.is_super_admin() OR
    EXISTS (SELECT 1 FROM public.events e WHERE e.id = event_id AND e.host_id = auth.uid())
  );
...
-- ─── RPC: batch reorder ────────────────────────────────────────────────────────
-- Called by reorderCompetitorsInGroup to avoid N round-trips.
...
-- Storage RLS: competitor can upload to their own folder; host can read all
-- (set in Dashboard → Storage → receipts → Policies, or via SQL below)
--
-- CREATE POLICY "receipts: competitor upload" ON storage.objects
--   FOR INSERT WITH CHECK (
--     bucket_id = 'receipts' AND auth.uid()::text = (storage.foldername(name))[1]
--   );
--
-- CREATE POLICY "receipts: host read" ON storage.objects
--   FOR SELECT USING (
